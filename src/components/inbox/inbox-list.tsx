...
import { statusLabels } from "@/utils/inbox";

type Props = {
  items: InboxItem[];
  filter: InboxFilter;
  isLoading?: boolean;
  onToggleDone: (item: InboxItem) => void;
  onMarkScheduled: (item: InboxItem) => void;
...
export function InboxList({
  ...
  return (
    <ul className="space-y-3">
      {filtered.map((item) => {
        const isProcessing = activeItemId === item.id;
        return (
          <li
            key={item.id}
          ...
          >
            <div className="flex flex-1 items-start gap-3">
              <input
                ...
                className="mt-1 size-5 rounded border-white/40 bg-transparent accent-emerald-400 disabled:opacity-40"
              />
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs text-white/60">
                  ...
                  <span>입력: {format(new Date(item.createdAt), "HH:mm")}</span>
                  {item.carriedOverAt && (
            ...
            <div className="flex flex-col items-start gap-2 sm:items-end">
              {item.status === "pending" && (
                <button
                  onClick={() => onMarkScheduled(item)}
                  disabled={isProcessing}
                  className="rounded-lg border border-emerald-400/50 px-3 py-1 text-sm text-emerald-200 transition hover:bg-emerald-400/10 disabled:opacity-40"
                >
                  캘린더에 배치하기
                </button>
              )}
